...
このディレクトリは GitHub Pages や静的ホスティング向けに用意された、PMTiles を使って DEM の 3D 表示とヒルシェードを確認するための小さなフロントエンドです。

使い方（ローカルで PMTiles を提供して 3D 表示を確認する手順）:

1. 事前準備: `pmtiles` CLI がインストールされていること（npm の pmtiles パッケージなど）。

2. PMTiles をサーブする（例: `docs/sample_webp.pmtiles` を提供する）:

```bash
# pmtiles CLI を使ってローカルでタイルを配信
pmtiles serve ../docs/sample_webp.pmtiles --port 3001
```
